...
if __name__ == "__main__":
    ...
    path       = r"U:\yakos\hep_A\PROD\Environment_B"
    gdb        = os.path.join(path,"Data_B","Homeless_Activity_CURRENT.gdb")
    originFC   = os.path.join(gdb,"Sites")
    destFC     = os.path.join(gdb,"Visits")
    configFile = os.path.join(path,"Scripts_B","Source_Code","config_file.ini")
    mxd        = os.path.join(path,"MXDs_B","sites_with_visits.mxd")
    
    print("Creating Relationship Class")

    arcpy.management.MakeFeatureLayer(originFC,"originView")
    arcpy.management.MakeTableView(destFC,"destView")

    try:
        if arcpy.Exists(os.path.join(gdb,"Sites_Visits")):
            arcpy.management.Delete(os.path.join(gdb,"Sites_Visits"))
            
        arcpy.management.CreateRelationshipClass("originView",
                                             "destView",
                                             "Sites_Visits",
                                             "SIMPLE",
                                             "Visits",
    ...
    # AGOL Credentials
    usr = config.get("AGOL","usr")
    pwd = config.get("AGOL","pwd")

    # FS values
    serviceName = "sites_with_visits"
    folderName  = "Homeless Activity B"
    tags        = "homeless encampment"
    summary     = "Homeless encamplent sites with the visit hisorty."
    maxRecords  = "5000"
    ...
    everyone = False
    orgs = False
    groups = "7d867d0352cc41b8963c6717812cf8ed" #Homeless Encampment Editor
