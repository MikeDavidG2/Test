#-------------------------------------------------------------------------------
# Name:        DEV_TRACKER_Process_Existing_DU_Map_03.py
# Purpose:
"""
...
def main():
    ...
    # Name of this script
    name_of_script = 'DEV_TRACKER_Process_Existing_DU_Map_03.py'
    ...
    try:
        QA_QC(csv_table, record_id_fld, apn_fld, x_fld, y_fld)
    ...
    #------------------------------------------------------------------
    #         Change field names so the merge below will work
    #------------------------------------------------------------------
    try:
        # We want to keep the Dwelling Unit info from the SANDAG data
        old_name = 'du'
        new_name = du_fld
        print '  Changing Field: "{}"\n  To:  "{}"\n  in FC:\n    {}'.format(old_name, new_name, SANDAG_LU_DU_subset)  # For testing purposes
        arcpy.AlterField_management(SANDAG_LU_DU_subset, old_name, new_name)


        # We want to keep the APN info from the SANDAG data
        old_name = 'apn8'
        new_name = apn_fld
        print '  Changing Field: "{}"\n  To:  "{}"\n  in FC:\n    {}'.format(old_name, new_name, SANDAG_LU_DU_subset)  # For testing purposes
        arcpy.AlterField_management(SANDAG_LU_DU_subset, old_name, new_name)
    ...
    try:
        ...
        merge_fc = os.path.join(wkg_fgdb, '{}_SANDAG_merge'.format(points_name))
        print 'Merging:'
        for f in inputs:
            print '  {}'.format(f)
